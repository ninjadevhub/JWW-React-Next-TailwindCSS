...
import { FaTimes,  FaAngleRight,FaAngleDown} from 'react-icons/fa'
 ...
 const InvolvedSubmitModal = ({data,onClose}) => {
    if(!data) {
        return null
    }
            ...
            <div className='absolute right-0 -top-4 cursor-pointer' onClick={onClose}>
                ...
                <div className='border-b-2 border-brand-green w-2/3 bg-brand-form-bg2 flex flex-col justify-center items-center'>
                    <div className='mt-8 mb-8'>
                        <p className="font-museo text-2xl text-brand-gray-typo">{data.form1ThankYouPopupText}</p>

                    </div>

                    <div className='pb-8'>
                        <button className="w-48 bg-brand-blue font-nova text-base m-auto text-white font-bold py-2 px-4 rounded">
                            {data.form1ThankYouPopupButtonText}
                        </button>
                    </div>
                </div>
...
export default InvolvedSubmitModal